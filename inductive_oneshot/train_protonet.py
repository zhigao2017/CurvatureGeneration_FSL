...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='convnet', 
                        choices=['convnet', 'resnet12','bigres12'])
    ...
    parser.add_argument('--divide', type=float, default=100)
    parser.add_argument('--load_init_weight', action='store_true', default=False)
    parser.add_argument('--init_weights_bigres12', type=str, default='/media/dist2T_1/hyperbolic-image-embeddings-master_CVPR2021submission/hyperbolic-image-embeddings-master_multidistanceFBPproto_metarerank_transductive_oneshot/examples/fewshot/models/newRes12-pre.pth')  
    parser.add_argument('--init_weights_resnet12', type=str, default='mini-resnet12.pth')    
    ...
    parser.add_argument('--multihead', default=1,   type=int, help='multi_head')
...
#     set_gpu(args.gpu)
    ...
    if args.save_path is None:
        ...
        save_path2 = '_'.join([str(args.shot), str(args.query), str(args.way), str(args.validation_way),
                               ...
                               str(args.temperature), str(args.dim), str(args.train_c),
                               str(args.train_x), str(args.model),str(args.l),str(args.divide),str(args.rerank),str(args.load_init_weight)])
    ...
    val_sampler = CategoriesSampler(valset.label, 1000, args.validation_way, args.shot + args.query)
    ...
    if args.model=='convnet':
        to_optim          = [{'params':model.encoder.encoder.parameters(),'lr':args.lr},
                             ...
                             {'params':model.proj_k.parameters(),'lr':args.c_lr,'weight_decay':args.p_decay},
                             {'params':model.proj_q.parameters(),'lr':args.c_lr,'weight_decay':args.p_decay},
                             {'params':model.proj_v.parameters(),'lr':args.c_lr,'weight_decay':args.p_decay},
                             {'params':model.fc_new.parameters(),'lr':args.c_lr,'weight_decay':args.p_decay},
                             {'params':model.layer_norm.parameters(),'lr':args.c_lr,'weight_decay':args.p_decay},
                             {'params':model.layer_norm2.parameters(),'lr':args.c_lr,'weight_decay':args.p_decay},
                             ]
    else:
    ...
    if args.load_init_weight:
        if args.model=='bigres12':
            pretrained_dict = torch.load(args.init_weights_bigres12)
            model.load_state_dict(pretrained_dict['params'],strict=False)
        if args.model=='resnet12':
            pretrained_dict = torch.load(args.init_weights_resnet12)
            model.load_state_dict(pretrained_dict['model_sd'],strict=False)
        model.cuda()
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            torch.nn.utils.clip_grad_value_(model.controller.parameters(),15)
            torch.nn.utils.clip_grad_value_(model.controller.parameters(),15)
