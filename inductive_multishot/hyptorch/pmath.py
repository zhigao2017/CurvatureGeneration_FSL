...
def lorenz_factor(x, *, c=1.0, dim=-1, keepdim=False):
    ...
    """
    #print('------------------------lorenz_factor------------------------------------')
    #print('lorenz_factor',torch.sqrt(   1 - c * x.pow(2).sum(dim=dim, keepdim=keepdim) ) )
    return 1 / torch.sqrt(1 - c * x.pow(2).sum(dim=dim, keepdim=keepdim))
    #return 1 / torch.sqrt( torch.clamp_min( 1 - c * x.pow(2).sum(dim=dim, keepdim=keepdim) , 1e-5  ) )
...
def scalar_mul_matrix( point, weight, c=1.0):

    sqrt_c = c ** 0.5

    #return ((1 / sqrt_c)*torch.tanh(weight*artanh(sqrt_c* torch.norm(point,dim=-1) ))/torch.norm(point,dim=-1) * point.t() ).t()
    #print('------------------------scalar_mul_matrix------------------------------------')
    #print('weight',weight)
    point_norm = torch.clamp_min(torch.norm(point,dim=-1), 1e-5)
    #print('point_norm',point_norm)
    #print('scalrmul',((1 / sqrt_c)*torch.tanh(weight*artanh(sqrt_c* point_norm ))/point_norm * point.t() ).t())
    return ((1 / sqrt_c)*torch.tanh(weight*artanh(sqrt_c* point_norm ))/point_norm * point.t() ).t()
